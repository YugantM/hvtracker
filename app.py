...
def badge_by_slug(slug: str):
    ...
    if slug.endswith("-grade"):
        ...
        # per-request like the score/grade badges. Serve the file from the
        # render output root (the volume in prod) — NOT BASE_DIR, which is
        # the immutable code dir and never receives rendered badges.
        base_slug = slug[:-6]
...
<head>
  ...
  <script>
    ...
    var ua=navigator.userAgent||"";if(navigator.webdriver||/HeadlessChrome|Puppeteer|Playwright|Claude\\/[\\d.]+.*Electron\\/|bot|crawl|spider|curl|wget|python-requests/i.test(ua)){{window["ga-disable-G-TZ8921LR0K"]=true}}}})();
  </script>
...
curl -sO https://hvtracker.net/data/exports/hvtrust-QUARTER_LABEL.csv</code></pre>
  ...
  "mcpServers": {
    "hvtracker": { "url": "https://hvtracker.net/mcp" }
      ...
      <p style='margin-top:12px;color:var(--muted);font-size:13px'>Tools: <code>verify_mcp_server</code> (pre-connect "Safe Browsing for MCP" verdict), <code>check_agent_trust</code> (incl. runtime capabilities + signed-credential URL), <code>scan_stack</code> (verdict for a whole requirements.txt / package.json / mcp config), <code>compare_agents</code>, <code>get_leaderboard</code>, <code>list_categories</code>, <code>get_agent_history</code> (90-day trust trend), <code>search_agents</code>.</p>
    </div>
    <div class='card'>
